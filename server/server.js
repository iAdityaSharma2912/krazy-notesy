...
// Serve Uploaded Files Statically using the absolute path
...
app.get('/api/files', (req, res) => {
  ...
  fs.readdir(UPLOADS_DIR, (err, files) => {
    ...
    // We explicitly use req.get('host') because the frontend expects the backend's host, not its own.
    ...
    const fileInfos = files.map((file) => {
      ...
      return {
        ...
        // Correct path: Base URL + /uploads/ + filename
...
app.get('/api/stats', (req, res) => {
  ...
  if (!fs.existsSync(directoryPath)) {
    return res.json({ totalMedia: 0, totalSize: '0 MB', activePosts: 0, engagement: '0' });
  }

  fs.readdir(directoryPath, (err, files) => {
    if (err) return res.json({ totalMedia: 0, totalSize: '0 MB', activePosts: 0, engagement: '0' });
