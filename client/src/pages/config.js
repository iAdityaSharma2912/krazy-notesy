...
import { Cog6ToothIcon, CheckCircleIcon, ArrowPathIcon, XMarkIcon } from '@heroicons/react/24/outline';
// Import Brand Icons
import { FaInstagram, FaYoutube, FaXTwitter, FaPinterest } from 'react-icons/fa6';
import axios from 'axios';

// --- LOGIN MODAL COMPONENT ---
const LoginModal = ({ isOpen, onClose, platform, onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Send Credentials to Backend for Verification
      // In a real app, this endpoint would run your Python script or use an API wrapper
      // const res = await axios.post('https://stupendously-unligatured-tamar.ngrok-free.dev/api/platforms/verify', {
      //   platform: platform.key,
      //   username,
      //   password
      // });

      // SIMULATION OF API VERIFICATION
      console.log(`Verifying ${platform.name} credentials for ${username}...`);
      await new Promise(resolve => setTimeout(resolve, 2000)); // Simulate network delay

      // Simulate a success scenario (In reality, check res.data.success)
      const isSuccess = true; 

      if (isSuccess) {
        onLogin(username); // Pass username back to parent on success
        setLoading(false);
        setUsername('');
        setPassword('');
      } else {
      ...
      <div className="bg-neutral-900 border border-neutral-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden relative">
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <h3 className="text-2xl font-black text-white">Connect {platform.name}</h3>
            <p className="text-neutral-400 text-sm mt-2">Enter your credentials to authorize Krazy Notesy.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-neutral-500 mb-1 uppercase">Username / Email</label>
              <input 
                type="text" 
                value={username}
                ...
                placeholder={`Your ${platform.name} ID`}
              />
            </div>
            ...
            >
              {loading ? "Verifying..." : `Log In & Connect`}
            </button>
          </form>
...
export default function Config() {
  ...
  const platforms = [
    { key: 'instagram', name: 'Instagram', icon: <FaInstagram className="w-8 h-8 text-pink-500"/> },
    { key: 'youtube', name: 'YouTube', icon: <FaYoutube className="w-8 h-8 text-red-500"/> },
    { key: 'twitter', name: 'X / Twitter', icon: <FaXTwitter className="w-8 h-8 text-white"/> },
    { key: 'pinterest', name: 'Pinterest', icon: <FaPinterest className="w-8 h-8 text-red-600"/> },
  ];
  ...
  // 3. Handle Successful Login from Modal
  const handleLoginSuccess = (username) => {
    if (!selectedPlatform) return;

    // Save the connection state
    const newConnections = { ...connections, [selectedPlatform.key]: true };
    setConnections(newConnections);
    localStorage.setItem('social_connections', JSON.stringify(newConnections));

    // Save a session token for this platform (Mock)
    localStorage.setItem(`${selectedPlatform.key}_session`, JSON.stringify({
      user: username,
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-3xl font-black text-white">Configurations</h2>
          <p className="text-neutral-400">Manage your social API connections.</p>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <p className="text-sm text-neutral-400 mb-6 ml-11 relative z-10">
                {isConnected 
                  ? "Session active. Your credentials are verified." 
                  : "Not connected. Link your account to enable auto-posting."}
              </p>

              {/* Action Button */}
              <button 
                onClick={() => isConnected ? handleDisconnect(platform.key) : initiateConnection(platform)}
