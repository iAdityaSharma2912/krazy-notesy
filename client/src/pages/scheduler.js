...
export default function Scheduler() {
  ...
  useEffect(() => {
    ...
    // 1. Get User Name and Connections from Local Storage (Client-side ONLY)
    ...
    if (savedConfig) setConnectedPlatforms(JSON.parse(savedConfig));
    
    // 2. Fetch Files
    axios.get(`${API_URL}/api/files`)
      .then(res => setMediaFiles(res.data))
      .catch(err => setLoadError("Failed to load files from API. Check Node server console."));
      
  }, []); 

  useEffect(() => {
    if (scheduleType !== 'fully-auto' && isAutoAiEnabled && selectedFiles.size > 0 && !isAiGenerating) {
        handleAiCaption();
  ...
  const handleAutoSelect = () => {
    ...
    const filesToSelect = mediaFiles.slice(0, 5).sort(() => 0.5 - Math.random()).slice(0, 3);
    filesToSelect.forEach(file => newSelection.add(file.id));
    setSelectedFiles(newSelection);
  };
  ...
  const handleSelectAllPlatforms = () => {
      const connectableKeys = platformOptions.filter(p => connectedPlatforms[p.id]).map(p => p.id);
      const allSelected = connectableKeys.length === selectedPlatforms.size;

      if (allSelected) {
          setSelectedPlatforms(new Set());
      } else {
          setSelectedPlatforms(new Set(connectableKeys));
      }
  };

  const handleAiCaption = () => {
    if (selectedFiles.size === 0 && scheduleType !== 'fully-auto') { alert("Please select a media file first so AI knows what to write about!"); return; }
    setIsAiGenerating(true);
    const firstFile = mediaFiles.find(f => selectedFiles.has(f.id));
    const mediaType = firstFile ? (firstFile.type === 'pictures' ? 'Photo' : 'Video/Reel') : 'Random Content';

    setTimeout(() => {
      let captionTemplates = [];
      if (mediaType === 'Photo') { captionTemplates = [`Just dropped this epic shot! 📸 #Photography`, `Captured the moment perfectly. #Visuals`]; } 
      else if (mediaType === 'Video/Reel') { captionTemplates = [`New ${mediaType} alert! Check out the full story. 🎥 #Shorts`, `Watch till the end! 🤖 #VideoOfTheDay`]; } 
      else { captionTemplates = [`General automated post. Chaos managed! #KrazyNotesy`]; }
      setCaption(captionTemplates[Math.floor(Math.random() * captionTemplates.length)]);
  ...
  const handleSchedule = async () => {
    // 1. Validation 
    if (selectedPlatforms.size === 0) { alert("Please select at least one platform to publish to."); return; }
    if (scheduleType === 'one-time' && (!date || !time)) { alert("Please select a date and time for the one-time post."); return; }
    if (!time) { alert("Please select a time for the automated post."); return; }
    if (scheduleType !== 'fully-auto' && selectedFiles.size === 0) { alert("Please select at least one file to post."); return; }

    setIsScheduling(true);
    
    const dateTimeString = scheduleType === 'one-time' ? `${date}T${time}:00` : `1970-01-01T${time}:00`;
    ...
    try {
        ...
        setLoadError(error.response?.data?.message || "Scheduling failed. API error.");
    } finally {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <div className="p-3 bg-yellow-400 rounded-xl text-neutral-900"><CalendarDaysIcon className="w-8 h-8" /></div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className={`bg-neutral-800 p-6 rounded-2xl border ${scheduleType === 'fully-auto' ? 'opacity-40' : 'border-neutral-700'}`}>
            ...
            : mediaFiles.length > 0 ? (<>
                <div className="mb-4 flex gap-3">
                    <button onClick={handleAutoSelect} className="flex items-center gap-2 px-3 py-1.5 bg-neutral-700 hover:bg-neutral-600 rounded-lg text-sm font-bold text-white transition-colors"><ArrowsRightLeftIcon className="w-4 h-4" /> Auto-Select (Random)</button>
                    <button onClick={() => setSelectedFiles(new Set())} className="flex items-center gap-2 px-3 py-1.5 bg-neutral-700/50 hover:bg-neutral-700 rounded-lg text-sm font-bold text-neutral-400 transition-colors"><XMarkIcon className="w-4 h-4" /> Clear Selection</button>
                </div>
                <div className="flex gap-4 overflow-x-auto pb-4">
                {mediaFiles.map((file) => {
                    const isSelected = selectedFiles.has(file.id);
                    return (<div key={file.id} onClick={() => toggleFileSelection(file.id)} className={`relative w-28 h-28 bg-neutral-900 rounded-xl border-2 cursor-pointer flex-shrink-0 overflow-hidden group transition-all ${isSelected ? 'border-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.3)]' : 'border-neutral-700 hover:border-neutral-500'}`}>
                        {file.type === 'image' || file.type === 'pictures' ? (<img src={file.url} alt={file.name} className="w-full h-full object-cover" />) : (<div className="w-full h-full flex items-center justify-center"><VideoCameraIcon className="w-8 h-8 text-neutral-600" /></div>)}
                        <div className="absolute top-1 right-1">{isSelected ? (<CheckCircleIcon className="w-6 h-6 text-yellow-400 bg-neutral-900 rounded-full" />) : (<CheckBadgeIcon className="w-6 h-6 text-neutral-400 opacity-0 group-hover:opacity-100 transition-opacity bg-neutral-900/50 rounded-full" />)}</div>
                        <div className="absolute bottom-0 left-0 right-0 bg-black/70 text-[10px] text-white p-1 truncate">{file.name}</div>
                    </div>)})}
          ...
          <div className="bg-neutral-800 p-6 rounded-2xl border border-yellow-400/50">
            ...
            <div className="flex gap-4 mb-4 bg-neutral-900 p-2 rounded-xl">
                ...
                <button onClick={() => setScheduleType('fully-auto')} className={`flex-1 px-4 py-2 rounded-lg text-sm font-bold transition-colors ${scheduleType === 'fully-auto' ? 'bg-purple-600 text-white shadow-lg' : 'text-neutral-500 hover:text-neutral-300'}`}>Fully Automated</button>
            </div>

            <div className="grid grid-cols-2 gap-4">
              {scheduleType === 'one-time' && (<div><label className="block text-xs text-neutral-400 mb-2">Date</label><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full bg-neutral-900 border border-neutral-700 text-white rounded-lg px-4 py-3 focus:border-yellow-400 outline-none transition-colors" /></div>)}
              <div className={scheduleType === 'one-time' ? '' : 'col-span-full'}><label className="block text-xs text-neutral-400 mb-2">Time ({scheduleTimeLabel})</label><div className="relative"><input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full bg-neutral-900 border border-neutral-700 text-white rounded-lg px-4 py-3 focus:border-yellow-400 outline-none transition-colors" /><ClockIcon className="w-5 h-5 text-neutral-500 absolute right-3 top-3.5 pointer-events-none" /></div></div>
            </div>
            ...
            <div className={`mt-6 pt-4 border-t border-neutral-700 ${scheduleType === 'one-time' ? '' : 'opacity-40'}`}>
                ...
                {minDelayHours > 0 && selectedFiles.size > 1 && (
                    <p className="text-xs text-yellow-500/80 mt-2">Posts will be automatically spaced {minDelayHours} hours apart.</p>
                )}
            </div>
          </div>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-8">
            ...
            <div className="bg-neutral-900 border-[8px] border-neutral-800 rounded-[2.5rem] overflow-hidden shadow-2xl h-[600px] relative flex flex-col">
              {successMsg && (<div className="absolute inset-0 z-50 bg-neutral-900/90 backdrop-blur-sm flex items-center justify-center p-6 text-center animate-in fade-in zoom-in"><div><CheckCircleIcon className="w-16 h-16 text-green-400 mx-auto mb-4" /><h3 className="text-xl font-bold text-white">Post Job Created!</h3><p className="text-neutral-400 mt-2">{successMsg}</p></div></div>)}
              <div className="bg-neutral-900 p-4 flex items-center gap-3 border-b border-neutral-800 shrink-0">
                ...
                <div><p className="text-xs font-bold text-white">{userName}</p><p className="text-[10px] text-neutral-400">
                    {scheduleType === 'fully-auto' ? 'FULLY AUTOMATED JOB' : Array.from(selectedPlatforms).join(', ').toUpperCase() || 'SELECT PLATFORMS'}
                </p></div>
              </div>
              <div className="flex-1 bg-neutral-800 flex items-center justify-center overflow-hidden relative">
                 {selectedPreviewFile ? (selectedPreviewFile.type === 'image' || selectedPreviewFile.type === 'pictures' ? (<img src={selectedPreviewFile.url} alt="Preview" className="w-full h-full object-contain" />) : (<video src={selectedPreviewFile.url} className="w-full h-full object-contain" preload="metadata" />)) 
                 : (<div className="text-center p-6"><PhotoIcon className="w-12 h-12 text-neutral-700 mx-auto mb-2" /><p className="text-xs text-neutral-600">
                           {scheduleType === 'fully-auto' ? 'Random Media Selection' : 'Select content to preview'}
                        </p></div>)}
              </div>
              <div className="p-4 bg-neutral-900 shrink-0">
                <div className="flex gap-4 mb-3">
                   <div className="w-5 h-5 rounded-full border border-neutral-600"></div><div className="w-5 h-5 rounded-full border border-neutral-600"></div><div className="w-5 h-5 rounded-full border border-neutral-600"></div>
                </div>
